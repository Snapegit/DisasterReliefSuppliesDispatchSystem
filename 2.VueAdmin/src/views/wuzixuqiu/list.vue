	...
	<div>
		<div class="app-contain">
			<div class="list_search_view">
					...
					<div class="search_view">
						...
						<div class="search_box">
							<el-select
								class="search_sel"
								clearable
								v-model="searchQuery.wuzizhonglei" 
								placeholder="物资种类"
								>
								<el-option v-for="item in wuzizhongleiLists" :label="item" :value="item"></el-option>
							</el-select>
						</div>
					</div>
					<div class="search_view">
						...
						<div class="search_box">
							<el-select
								class="search_sel"
								clearable
								v-model="searchQuery.jinjichengdu" 
								placeholder="紧急程度"
								>
								<el-option v-for="item in jinjichengduLists" :label="item" :value="item"></el-option>
							</el-select>
						</div>
					</div>
					<div class="search_view">
				...
				<br>
				<div class="btn_view">
					<el-button type="success" @click="addClick" v-if="btnAuth('wuzixuqiu','新增')">新增</el-button>
					<el-button  v-if=" btnAuth('wuzixuqiu','查看')" type="info"  :disabled="selRows.length==1?false:true" @click="infoClick(null)">详情</el-button>
					<el-button type="primary" :disabled="selRows.length==1?false:true" @click="editClick" v-if=" btnAuth('wuzixuqiu','修改')">修改</el-button>
					<el-button type="danger" :disabled="selRows.length?false:true" @click="delClick(null)"  v-if="btnAuth('wuzixuqiu','删除')">删除</el-button>
				</div>
			</div>
			<br>
			<el-table
				...
				<el-table-column label="审核" v-if="btnAuth('wuzixuqiu','审核')" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
				...
				<el-table-column label="操作" width="300" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
						<el-button type="info" v-if=" btnAuth('wuzixuqiu','查看')" @click="infoClick(scope.row.id)">详情</el-button>
						<el-button v-if="btnAuth('wuzixuqiu','调度')" type="success" @click="wuzidiaoduCrossAddOrUpdateHandler(scope.row,'cross','是','wuzizhuangtai','已调度','已调度,未处理'.split(',')[0])">调度</el-button>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination 
		...
		<wuzidiaoduFormModel ref="wuzidiaoduFormModelRef" @formModelChange="formModelChange"></wuzidiaoduFormModel>
	</div>
</template>
<script setup>
	...
	const getList = () => {
		...
		params['sort'] = 'id'
		params['order'] = 'desc'
		if(searchQuery.value.wuzizhonglei&&searchQuery.value.wuzizhonglei!=''){
			params['wuzizhonglei'] = searchQuery.value.wuzizhonglei
		}
		if(searchQuery.value.jinjichengdu&&searchQuery.value.jinjichengdu!=''){
			params['jinjichengdu'] = searchQuery.value.jinjichengdu
		}
	...
	getwuzizhongleiLists()
	const jinjichengduLists = ref([])
	const getjinjichengduLists = () => {
		jinjichengduLists.value = '一级,二级,三级'.split(',')
	}
	getjinjichengduLists()
	const searchClick = () => {
	...
	const approvalClick = (row) => {
		let params = {
			...
			suoxushuliang: row.suoxushuliang,
			zaiqingmiaoshu: row.zaiqingmiaoshu,
			jinjichengdu: row.jinjichengdu,
			...
			sfsh: row.sfsh,
			shhf: row.shhf,
			wuzizhuangtai: row.wuzizhuangtai,
		}
		nextTick(() => {
	...
	import wuzidiaoduFormModel from '@/views/wuzidiaodu/formModel'
	const wuzidiaoduFormModelRef = ref(null)
    const wuzidiaoduCrossAddOrUpdateHandler = (row,type,crossOptAudit,statusColumnName,tips,statusColumnValue) => {
		...
		nextTick(()=>{
			wuzidiaoduFormModelRef.value.init(row.id,'cross','调度',row,'wuzixuqiu',statusColumnName,tips,statusColumnValue)
		})
    }
	//查询审核状态列表
