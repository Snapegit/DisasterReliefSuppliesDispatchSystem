	...
	<div>
		<div class="app-contain">
			<div class="list_search_view">
				<el-form :model="searchQuery" class="search_form" >
					<div class="search_view">
						...
						<div class="search_box">
							<el-input class="search_inp" v-model="searchQuery.diaodubianhao" placeholder="调度编号"
								clearable>
							</el-input>
						</div>
					</div>
				...
				<br>
				<div class="btn_view">
					<el-button type="success" @click="addClick" v-if="btnAuth('wuzidiaodu','新增')">新增</el-button>
					<el-button  v-if=" btnAuth('wuzidiaodu','查看')" type="info"  :disabled="selRows.length==1?false:true" @click="infoClick(null)">详情</el-button>
					<el-button type="primary" :disabled="selRows.length==1?false:true" @click="editClick" v-if=" btnAuth('wuzidiaodu','修改')">修改</el-button>
					<el-button type="danger" :disabled="selRows.length?false:true" @click="delClick(null)"  v-if="btnAuth('wuzidiaodu','删除')">删除</el-button>
				</div>
			</div>
			<br>
			<el-table
				...
				<el-table-column label="操作" width="300" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
						<el-button type="info" v-if=" btnAuth('wuzidiaodu','查看')" @click="infoClick(scope.row.id)">详情</el-button>
						<el-button v-if="btnAuth('wuzidiaodu','更新进度')" type="success" @click="diaodujinduCrossAddOrUpdateHandler(scope.row,'cross','','','')">更新进度</el-button>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination 
		...
		<diaodujinduFormModel ref="diaodujinduFormModelRef" @formModelChange="formModelChange"></diaodujinduFormModel>
	</div>
</template>
<script setup>
	...
	const getList = () => {
		...
		params['sort'] = 'id'
		params['order'] = 'desc'
		if(searchQuery.value.diaodubianhao&&searchQuery.value.diaodubianhao!=''){
			params['diaodubianhao'] = '%' + searchQuery.value.diaodubianhao + '%'
		}
	...
	import diaodujinduFormModel from '@/views/diaodujindu/formModel'
	const diaodujinduFormModelRef = ref(null)
    const diaodujinduCrossAddOrUpdateHandler = (row,type,crossOptAudit,statusColumnName,tips,statusColumnValue) => {
		...
		nextTick(()=>{
			diaodujinduFormModelRef.value.init(row.id,'cross','更新进度',row,'wuzidiaodu',statusColumnName,tips,statusColumnValue)
		})
    }
	//初始化
	const init = () => {
