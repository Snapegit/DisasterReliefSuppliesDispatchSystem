		...
		<el-dialog v-model="formVisible" :title="formTitle" width="80%" destroy-on-close :fullscreen='false'>
			<el-form class="formModel_form" ref="formRef" :model="form" label-width="$template2.back.add.form.base.labelWidth" :rules="rules">
				<el-row>
					...
					<el-col :span="24">
						<el-form-item prop="wuzitupian"
									  label="物资图片"
						>
							<uploads
								:disabled="!isAdd||disabledForm.wuzitupian?true:false"
								action="file/upload"

								tip="请上传物资图片"
								:limit="3"
								style="width: 100%;text-align: left;"
								:fileUrls="form.wuzitupian?form.wuzitupian:''" 
								@change="wuzitupianUploadSuccess">
							</uploads>
						</el-form-item>
					</el-col>
					<el-col :span="12">
...
<script setup>
	...
	//物资图片上传回调
	const wuzitupianUploadSuccess=(e)=>{
		form.value.wuzitupian = e
	}
	//methods
	...
	const init=(formId=null,formType='add',formNames='',row=null,table=null,statusColumnName=null,tips=null,statusColumnValue=null)=>{
		...
		if(formType == 'add'){
			...
			for(let x in row){
				...
				if(x=='wuzitupian'){
					form.value.wuzitupian = row[x];
					disabledForm.value.wuzitupian = true;
					continue;
				}
				if(x=='shuliang'){
	...
	//提交
	const save=()=>{
		if(form.value.wuzitupian!=null) {
			form.value.wuzitupian = form.value.wuzitupian.replace(new RegExp(context?.$config.url,"g"),"");
		}
